...
LinearInterpolation = function(mobmat){
  # replace 4s with 1s
  ID4=which(mobmat[,1]==4)
  if(length(ID4)==0){return(mobmat)}
  for(ID in ID4){
    mobmat[ID,1]=1
    if(mobmat[ID-1,1]==1){
      mobmat[ID,2]=mobmat[ID-1,5]
      mobmat[ID,3]=mobmat[ID-1,6]
    }else{
      mobmat[ID,2]=mobmat[ID-1,2]
      mobmat[ID,3]=mobmat[ID-1,3]
    }
    mobmat[ID,5]=mobmat[ID+1,2]
    mobmat[ID,6]=mobmat[ID+1,3]
  }
  # eliminate 3s
  ID3=which(mobmat[,1]==3)  
  if(length(ID3)>0){
    mobmat=mobmat[-ID3,]
  }
  return(mobmat)
...
SimulateMobilityGaps <-
  function(suboutmat,obj,wtype="TL",spread_pars=c(1,1)){
    if(length(which(suboutmat[,1]==1))==0){
      return(LinearInterpolation(suboutmat))
    }
