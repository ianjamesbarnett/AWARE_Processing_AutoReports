...
AccelerometerDataCollection = function(ACCvec,tol){
  ...
  outmat=matrix(outmat[1:counter,],ncol=2)
  if(counter<2 && length(ACCvec)<2){
    return(NULL)
  }
  if(counter<2 && length(ACCvec)>2){
    outmat[1,2]=ACCvec[length(ACCvec)]
    return(outmat*1000)
  }
...
GPSDataCollection = function(GPSmat,tol=5){
  ID4=which(GPSmat[,1]>=4)
  if(length(ID4)>0){
    GPSmat = GPSmat[-ID4,]
  }
  ID3=which(GPSmat[,1]==3)
  if(length(ID3)>0){
    GPSmat[ID3,7]=GPSmat[ID3,4]
  }
  outmat2 = matrix(NA,nrow=100,ncol=2)
  ...
  outmat2 = outmat2[1:counter,]
  outmat2[counter,2]=GPSmat[nrow(GPSmat),7]
  if(counter<2 && nrow(GPSmat)-length(ID4)<2){
    return(NULL)
  }
  if(counter<2 && nrow(GPSmat)-length(ID4)>=2){
    return(1000*matrix(c(GPSmat[1,4],GPSmat[nrow(GPSmat),7]),ncol=2))
  }
...
PlotContinuousDataCollectionTracks=function(homedir,ID,ACCmat,GPSmat,SCNmat,DZEmat,SVYvec,tz=""){
  ...
  for(ii in 1:nweeks){
    ...
    if(ACCID2>ACCID1){
      ...
      subACCmat=matrix(ACCmat[ACCID1,],ncol=2)
    }
    ...
    if(GPSID2>GPSID1){
      ...
      subGPSmat=matrix(GPSmat[GPSID1,],ncol=2)
    }
...
SCNDataCollection = function(homedir,ID,ACCmat,GPSmat,tol=10*60){
  ...
  scndir = paste(homedir,"/",ID,"/screen-",substr(ID,0,8),".csv",sep="")
  if(!file.exists(scndir)){return(NULL)}
