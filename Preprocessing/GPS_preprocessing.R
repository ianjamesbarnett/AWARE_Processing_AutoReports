GPS_preprocessing = function(patient_name,
  ...
  if(file.exists(patient_output_filename)){
    ...
    fileinname=paste(data_filepath,"/",patient_name,"/location-",substr(patient_name,0,8),".csv",sep="")
#    filelist <- list.files(path=paste(data_filepath,patient_name,sep="/"),pattern = "\\.csv$",full.names=T)
#    if(length(filelist)==0){return(NULL)}
    mobmatmiss=GPS2MobMat(fileinname,itrvl=ITRVL,accuracylim=ACCURACY_LIM,r=rad_fp,w=wid_fp)
    ...
    if(qOKmsg=="Mobmat not a matrix. Removing individual from analysis.\n"){
...
GPS2MobMat = function(file,itrvl=10,accuracylim=51,r=NULL,w=NULL,tint_m=NULL,tint_k=NULL){
  if(is.null(r)){
    r=sqrt(itrvl)
  }
#  count=0
...
#    count=count+1
    ...
    dims=nrow(dat)
    mat=dat[,c(1,3,4,5,6)] # ignore UTC.time
#  }
